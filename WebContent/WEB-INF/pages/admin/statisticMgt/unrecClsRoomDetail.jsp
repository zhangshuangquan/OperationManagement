...
<h3 id="cataMenu">
	...
	<a href="javascript:;">
		<c:if test="${type eq 'SCH' }">
		   ${schoolInfo.schoolName }听课详情
		</c:if>
		<c:if test="${type ne 'SCH' }">
		   ${areaInfo.areaName }听课详情
		</c:if>
	</a>
</h3>		
...
<div id="control">
	...
	<c:if test="${classroomType eq 'RECEIVE' && classType ne 'Y'}">
		<h3 class="cataMenu navMenu mb10">
			<span class="tabMenu">
				<a class="tabItem" href="javascript:switchTab(0)">开课统计</a> 
				<a class="tabItem selected" href="javascript:switchTab(3)">未听课的接收教室</a>
			</span>
		</h3>
	</c:if>
...
<script type="text/javascript">
	...
	function search(){
		var url  = "${root}/admin/statisticsAnaly/getUnrecClsroomDetail.do";	
		var param  = {
	...
	function exportData(){
		$("#tempDataForm").attr("action","exportUnrecClsroomDetail.html");
		$("#tempDataForm").submit();
	}
	 ...
	 $(document).ready(function(){
		 ...
		 $("#query").click(search);	 
		   ...
		   mySplit = new SplitPage({
			    node : $id("pageNavi"),
			    url : "${root}/admin/statisticsAnaly/getUnrecClsroomDetail.do",
			    data:{
			    	classroomType:classroomType,
			    	classType:classType,
					startDt:startDt,
...
<form id="tempDataForm" action="exportUnrecClsroomDetail.html" method="post">
