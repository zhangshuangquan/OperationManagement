...
<form id="editForm">
    <ul class="commonUL" style="margin:5px 0 15px;">
        ...
        <li class="clearfix">
        	<label class="text">项目：</label>
				<span class="cont" id="chooseProject">
					<select class="mr20" id="project">
						<option value="${classroom.projectId }">${classroom.projectName }</option>
					</select>
				</span>
        </li>
        	...
        	<label class="text">学校：</label>
				<span class="cont" id="chooseSchool">
					<select class="mr20" id="school" name="clsSchoolId" needcheck nullMsg="请选择学校!" >
						<option value="${classroom.clsSchoolId }">${classroom.clsSchoolName }</option>
					</select>
				</span>
        </li>
        <li class="clearfix" style="margin-bottom:5px;">
            <label class="text">教室：</label>
                <div style="display:inline-block;width:161px;vertical-align:top;">
                    <input type="text" name="roomName" id="roomName"  needcheck nullMsg="请输入教室名字!"  value="${classroom.roomName}">
               	</div>&nbsp;&nbsp;
        </li>
         <li class="clearfix" style="margin-bottom:5px;">
            <label class="text">教室类别：</label>
                <span class="cont" id="chooseClassroomType">
					<select class="mr20"  name="roomType" needcheck nullMsg="请输入教室类别!">
						...
						<c:if test="${classroom.roomType == '主讲教室' }">
							<option value="主讲教室" selected="selected">主讲教室 </option>
							<option value="接收教室">接收教室 </option>
						</c:if>
						<c:if test="${classroom.roomType == '接收教室' }">
							<option value="主讲教室">主讲教室 </option>
							<option value="接收教室" selected="selected">接收教室 </option>
						</c:if>
					</select>
				</span>
        </li>
...
var mySplit = parent.mySplit;
	new BasicCheck({
	    ...
	    ajaxReq : function(){
	        $.post('${root}/admin/classroom/finishedit.do',$("#editForm").serialize(),function(r){
	            try {
	                if(!r.result){
	                    Win.alert('操作失败！');
	                }else{
