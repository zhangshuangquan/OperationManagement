...
<body>
	<form id="adminUser">
		...
		<ul class="commonUL">
		        ...
		        <span class="cont">
		        	 <input type="text" id="userName" name="userName" value="${adminUser.userName }" class="mr20" disabled="disabled" />
		        </span>
	       	</li>
				<li>
		        <label class="text">密码：</label>
		        <span class="cont">
		        	 <input type="text" id="password" name="password" value="" class="mr20" needcheck allownull limit="6,18" limitmsg="很抱歉，密码长度需要是6到18个英文字符" reg="^[0-9a-zA-Z|,|.|;|~|!|@|@|#|$|%|\^|&|*|(|)|_|+|-|=|\\|/|<|>]{6,18}$" errormsg="对不起，密码请输入英文字母、数字、符号（除特殊字符），或组合。" />
	       	...
	       	<li>
		        ...
		        <span class="cont">
		        	<input type="text" id="remark" value="${adminUser.remark }" name="remark" needcheck allownull limit="0,40" limitmsg="很抱歉，备注最大长度为20个字!" />
		        </span>
	       	</li>
	       	<li>
			     <label class="text">姓名：</label>
		        <span class="cont">
			        <input type="text" id="realName" name="realName" value="${adminUser.realName }" class="mr20" needcheck nullmsg="请输入姓名!" limit="1,20" limitmsg="很抱歉，姓名最大长度为10个字!" />
		         </span>
	       	</li>
	       	<li>
       			<label class="text">联系电话：</label>
       			<span class="cont">
		       		<input type="text" id="contact" value="${adminUser.contact }" name="contact" needcheck nullmsg="请输入联系电话!" reg="^\d{7,8}$|^\d{3,4}-\d{7,8}$|^1[3|4|5|8][0-9]\d{4,8}$" errormsg="联系电话格式不正确！" />
		       	</span>
	       	</li>
	       	<li>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("input[name=adminUserRoles]").each(function() {
			var id = $(this).attr("id") ;
			<c:forEach items="${adminUser.adminUserRoles }" var="adminRole">
				if('${adminRole.adminRoleId}' == id) {
