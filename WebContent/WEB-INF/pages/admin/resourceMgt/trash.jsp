...
<h3 id="cataMenu">
	...
	<a href="javascript:;"><c:if test="${column eq 'video' }">视频</c:if><c:if test="${column ne 'video' }">文档</c:if>资源回收站</a>
</h3>
<div id="control">
...
<script type="text/javascript">
		var mySplit = new SplitPage({
		    node : $id("pageNavi"),
		    url : "${root}/admin/resourceMgt/${column}/listResResourceTrash.do",
		    data:{
					categoryId:'0',
					semesterId:'0',
					classlevelId:'0',
		...
		function showList(data,totalCnt){
			...
			if(len>0){
				for(var i = 0; i< len; i++){
					...
					html += '<a href="javascript:;" class="recoveryLink" lang="'+resObj.resourceId+'">恢复</a>&nbsp;&nbsp;';
					html += '<a href="javascript:;" class="delLink" lang="'+resObj.resourceId+'">彻底删除</a>';
					html += '</td>';
					html += '</tr>';
				}
			} else {
				html += '<tr><td colspan="7">抱歉！没有搜索到您想要的信息！</td></tr>';
			}
		...
		function search(){
			var url  = "${root}/admin/resourceMgt/${column}/listResResourceTrash.do";
		...
		function bindEvent(){
            ...
            $(".tableBox").on("click",".recoveryLink",function(){
            	var $resId = $(this).attr("lang");
            	recoveryResource($resId);				
			});
		...
		function recoveryResource(resourceId){
			Win.confirm({mask:true,html:"确定要恢复吗?"},function(){
				var url = "${root}/admin/resourceMgt/${column}/recoveryResource.do?resourceId="+resourceId;
				$.ajax({
					   type: "get",
					   url: url,
		...
		function delResource(resourceId){
			Win.confirm({mask:true,html:"确定要删除吗?"},function(){
				var url = "${root}/admin/resourceMgt/${column}/deleteResourcePhysical.do?resourceIds="+resourceId;
				$.ajax({
					   type: "get",
					   url: url,
					   success: function(msg){
		...
		function delResourceBatch(){
			...
			if(len == 0) {
				...
				Win.confirm({mask:true,html:"确定要删除吗?"},function(){
					...
					$.ajax({                                                                                                                                            
						url		:	'${root}/admin/resourceMgt/${column}/deleteResourcePhysical.do',                                                                    
						cache	:	false,                                                                                                                                     
